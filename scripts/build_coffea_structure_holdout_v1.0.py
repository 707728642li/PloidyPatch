...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(values) != 1:
        raise SystemExit("usage: build_coffea_structure_holdout_v1.0.py PROJECT_ROOT")
    ...
    source_gff = input_root / "normalized/target_complete/primary_chromosomes.gff3"
    source_genome = input_root / "normalized/target_complete/primary_chromosomes.genome.fa"
    pair_tsv = evaluator_root / "truth_pairs/pairs.tsv"
    verify_sha256sums(input_root, ignore_checksum_file=True)
    for path in (project_root, input_root, evaluator_root):
        if not path.is_dir() or path.is_symlink():
            raise ValueError(f"Coffea holdout root is missing or symlinked: {path}")
    for path in (source_gff, source_genome, pair_tsv):
        if not path.is_file() or path.is_symlink() or path.stat().st_size == 0:
            raise ValueError(f"Coffea holdout input is missing: {path}")
    for path in (blind_output, evaluator_output):
        if path.exists() or path.is_symlink():
            raise FileExistsError(f"Refusing to overwrite Coffea holdout output: {path}")
    ...
    try:
        ...
        pair_selection.mkdir(); truth_root.mkdir(); validation.mkdir(); evaluator_inputs.mkdir()
        with pair_tsv.open(encoding="utf-8", newline="") as handle:
            available = len(list(csv.DictReader(handle, delimiter="\t")))
        if available < 1:
            raise ValueError("Coffea frozen pair rule produced zero target events")
        ...
        if not perturbation_manifest.is_file():
            raise ValueError("Coffea perturbation manifest is missing")
        ...
        groups = Counter(row["homoeolog_group"] for row in selected_rows)
        bins = Counter(row["coding_complexity_bin"] for row in selected_rows)
        complexity = {name: bins.get(name, 0) for name in EXPECTED_BINS}
        sentinels = {
            "blind_noop_exact_recovery": noop.get("event_recovery", {}).get(
                "complete_cds_chain_recovery"
            ),
            "complete_oracle_exact_recovery": oracle.get("event_recovery", {}).get(
                "complete_cds_chain_recovery"
            ),
        ...
        sentinel_valid = (
            ...
            and all(
                ...
                if key not in {"blind_noop_exact_recovery", "complete_oracle_exact_recovery"}
            )
        )
        data_gates = {
            "minimum_events": event_count >= MINIMUM_EVENTS,
            "minimum_target_chromosomes": len(chromosomes) >= MINIMUM_CHROMOSOMES,
            "minimum_homoeolog_groups": len(groups) >= MINIMUM_HOMOEOLOG_GROUPS,
        ...
        if status == "invalid":
            reason_codes = ["holdout_sentinel_failure"]
            ...
            if not data_gates["minimum_target_chromosomes"]:
                reason_codes.append("target_primary_chromosome_count_below_17")
            if not data_gates["minimum_homoeolog_groups"]:
                reason_codes.append("homoeolog_group_count_below_9")
            for name in EXPECTED_BINS:
                if complexity[name] < MINIMUM_PER_BIN:
                    reason_codes.append(f"complexity_bin_{name}_below_20")
        evaluability = {
            "schema_version": "ploidypatch.coffea_h1_evaluability.v1.0",
            ...
            "homoeolog_groups": len(groups),
            "events_by_target_chromosome": dict(sorted(chromosomes.items())),
            "events_by_homoeolog_group": dict(sorted(groups.items())),
        ...
        write_json(
            ...
            {
                "schema_version": "ploidypatch.coffea_h1_blind_benchmark_input.v1.0",
                "holdout_id": HOLDOUT_ID,
                "policy_id": POLICY_ID,
                "truth_access": False,
        ...
        write_json(
            ...
            {
                "schema_version": "ploidypatch.coffea_h1_structure_holdout.v1.0",
                "holdout_id": HOLDOUT_ID,
                "policy_id": POLICY_ID,
                "status": status,
        ...
        write_sha256sums(blind_work); write_sha256sums(evaluator_work)
        verify_sha256sums(blind_work, ignore_checksum_file=True)
        verify_sha256sums(evaluator_work, ignore_checksum_file=True)
        os.replace(blind_work, blind_output); os.replace(evaluator_work, evaluator_output)
