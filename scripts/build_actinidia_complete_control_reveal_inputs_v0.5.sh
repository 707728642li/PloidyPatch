...
holdout_contract=${PLOIDYPATCH_HOLDOUT_CONTRACT:-$project_root/config/holdouts/actinidia_red5_v0.5/contract.json}
protocol_root=${PLOIDYPATCH_PROTOCOL_FREEZE:-$project_root/results/protocol_freezes/actinidia_external_v0.5}
execution_root=${PLOIDYPATCH_EXECUTION_FREEZE:-${PLOIDYPATCH_EXECUTION_FREEZE_OVERRIDE:-$project_root/results/protocol_freezes/actinidia_external_v0.5_execution}}
composite_model_freeze=${PLOIDYPATCH_COMPOSITE_MODEL_FREEZE:-$execution_root/composite_model_freeze.json}
source_root=${PLOIDYPATCH_EVALUATOR_ONLY_ROOT:-$project_root/data/derived/external_inputs/actinidia/v0.5}
holdout_contract=$(realpath "$holdout_contract")
protocol_root=$(realpath "$protocol_root")
execution_root=$(realpath "$execution_root")
source_root=$(realpath "$source_root")
source_gff=$source_root/normalized/target_red5/primary_chromosomes.gff3
source_genome=$source_root/normalized/target_red5/primary_chromosomes.genome.fa
benchmark=$project_root/benchmark/structure/copy_collapse_v0.5/red5_ps1_1.69.0/annotation_copy_collapse_seed20261010
...
blind_run_root=${PLOIDYPATCH_BLIND_RUN_ROOT:-$project_root/results/blind_runs/actinidia_external_v0.5}
blind_run_root=$(realpath "$blind_run_root")
blind_project=$blind_run_root/project
upstream=$blind_project/results/baselines/actinidia_v0.5
method_root=$blind_project/results/copy_collapse/external/actinidia_v0.5_method_trio
ranking_root=$blind_project/results/copy_collapse/external/actinidia_v0.5_blind_rankings
blind_target_genome=$upstream/miniprot/normalized/target/primary_chromosomes.genome.fa
custody=${PLOIDYPATCH_CUSTODY_MANIFEST:-$blind_run_root/custody_manifest.json}
custody=$(realpath "$custody")
...
result_root=${PLOIDYPATCH_REVEAL_INPUTS_OUTPUT:-$project_root/results/evaluator/actinidia/v0.5/reveal_inputs}
working_root=${result_root}.working
self_relative=scripts/build_actinidia_complete_control_reveal_inputs_v0.5.sh

[[ -n ${PLOIDYPATCH_BLIND_RUN_ROOT:-} && -n ${PLOIDYPATCH_REVEAL_AUTHORIZATION:-} \
    && -n ${PLOIDYPATCH_HOLDOUT_CONTRACT:-} && -n ${PLOIDYPATCH_PROTOCOL_FREEZE:-} \
    && -n ${PLOIDYPATCH_EXECUTION_FREEZE:-} && -n ${PLOIDYPATCH_CUSTODY_MANIFEST:-} \
    && -n ${PLOIDYPATCH_COMPOSITE_MODEL_FREEZE:-} \
    && -n ${PLOIDYPATCH_EVALUATOR_ONLY_ROOT:-} && -n ${PLOIDYPATCH_REVEAL_INPUTS_OUTPUT:-} ]] || {
    echo "builder requires the standard frozen reveal environment" >&2; exit 1;
}
...
for required in "$python_bin" "$holdout_contract" "$protocol_root/SHA256SUMS" \
                "$policy" \
...
[[ -e $composite_model_freeze ]] || {
    echo "missing frozen composite model binding: $composite_model_freeze" >&2; exit 1;
}
...
if [[ -d $composite_model_freeze ]]; then
    verify_tree "$composite_model_freeze"
fi
...
[[ $(policy_value policy_id) == ploidypatch_actinidia_external_validation_v0.5 \
    ...
    echo "complete-control adaptation differs from the frozen Actinidia policy" >&2; exit 1;
}

"$python_bin" - "$authorization" "$blind_run_root/SHA256SUMS" "$custody" \
    "$blind_project" "$blind_scores" "$blind_score_manifest" "$pool_decisions" "$pool_manifest" \
    "$execution_root/SHA256SUMS" "$protocol_root/SHA256SUMS" \
    "$composite_model_freeze/SHA256SUMS" <<'PY'
import hashlib
import json
import sys
...
(
    authorization_path, blind_sums, custody_path, blind_project, scores,
    score_manifest, decisions, pool_manifest, execution_sums, protocol_sums,
    model_sums,
) = map(Path, sys.argv[1:])
...
expected_authorization = {
    ...
    "execution_SHA256SUMS_sha256": sha256(execution_sums),
    "protocol_SHA256SUMS_sha256": sha256(protocol_sums),
    "composite_model_SHA256SUMS_sha256": sha256(model_sums),
}
if (
    custody.get("schema_version") != "ploidypatch.external_holdout_blind_custody.v0.5"
    or custody.get("holdout_id") != "actinidia_red5_v0.5"
    or custody.get("policy_id") != "ploidypatch_actinidia_external_validation_v0.5"
    or any(custody.get(field) is not False for field in forbidden)
    or not custody.get("frozen_before_truth_reveal_at")
    or authorization.get("schema_version") != "ploidypatch.external_holdout_reveal_authorization.v0.5"
    or authorization.get("holdout_id") != "actinidia_red5_v0.5"
    or authorization.get("policy_id") != "ploidypatch_actinidia_external_validation_v0.5"
    or authorization.get("truth_reveal_authorized") is not True
    or authorization.get("authorized_statuses") != [
        "ready_for_evaluation", "not_evaluable_without_rule_relaxation", "invalid_run"
    ]
...
for name, expected_path in expected_outputs.items():
    ...
    if not isinstance(item, dict) or set(item) != {"relative_path", "sha256"}:
        raise SystemExit(f"malformed generic custody blind output: {name}")
    relative = Path(item["relative_path"])
    resolved = (blind_project / relative).resolve()
    try:
        resolved.relative_to(blind_project.resolve())
    except ValueError:
        raise SystemExit(f"custody blind output escapes blind project: {name}") from None
    if resolved != expected_path.resolve() or item["sha256"] != sha256(expected_path):
        raise SystemExit(f"custody blind output differs from canonical path: {name}")
...
for required in "$source_root/EVALUATOR_SHA256SUMS" "$source_gff" "$source_genome" \
                ...
                "$upstream/miniprot/reference/actinidia_candidate_refs.map.tsv" \
...
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Actinidia reveal inputs" >&2; exit 1;
}
...
binding_sha256() {
    local path=$1
    if [[ -d $path ]]; then sha256sum "$path/SHA256SUMS" | awk '{print $1}'
    else sha256sum "$path" | awk '{print $1}'
    fi
}
{
    printf 'field\tvalue\n'
    ...
    printf 'holdout_contract_sha256\t%s\n' "$(binding_sha256 "$holdout_contract")"
    printf 'composite_model_freeze_sha256\t%s\n' "$(binding_sha256 "$composite_model_freeze")"
} > "$working_root/run_contract.tsv"
cp -- "$evaluability" "$working_root/evaluability.json"
...
write_manifest() {
    ...
    "$python_bin" - "$working_root" "$status" "$reason" \
        ...
        "$blind_scores" "$blind_score_manifest" "$pool_decisions" "$pool_manifest" \
        "$holdout_contract" "$composite_model_freeze" \
...
(
    ...
    pool_manifest, holdout_contract, composite_model_freeze,
    benchmark_sums, truth, evaluability, authorization,
...
def sha256(value: str | Path) -> str:
    ...
    return digest.hexdigest()
manifest = {
    "schema_version": "ploidypatch.actinidia_reveal_inputs.v0.5",
    ...
    "holdout_contract_sha256": sha256(holdout_contract),
    "composite_model_freeze_sha256": (
        sha256(Path(composite_model_freeze) / "SHA256SUMS")
        if Path(composite_model_freeze).is_dir()
        else sha256(composite_model_freeze)
    ),
    "target_event": "Actinidia_specific_Ad_alpha",
    "evaluability": {
        "relative_path": "evaluability.json",
        "sha256": sha256(root / "evaluability.json"),
    },
...
with (root / "status.json").open("x", encoding="utf-8") as handle:
    json.dump(
        {
            "schema_version": "ploidypatch.actinidia_reveal_input_status.v0.5",
            "status": status,
            "formal_status": status,
            "holdout_id": "actinidia_red5_v0.5",
            "policy_id": "ploidypatch_actinidia_external_validation_v0.5",
...
finalize_tree() {
    ...
    PYTHONPATH="$code_root/src" "$python_bin" - "$working_root" <<'PY'
import sys
from pathlib import Path
...
if [[ $formal_outcome == invalid_run ]]; then
    ...
    finalize_tree
    printf 'Actinidia reveal builder retained fixed-rule non-evaluable outcome: %s\n' "$result_root"
    exit 0
elif [[ $formal_outcome != formally_evaluable_pending_blind_and_complete_control_reveal ]]; then
    echo "unrecognized Actinidia benchmark outcome: $formal_outcome" >&2
    exit 1
fi

cd "$code_root"
miniprot_gff=$upstream/miniprot/raw/miniprot.gff3
protein_map=$upstream/miniprot/reference/actinidia_candidate_refs.map.tsv
...
finalize_tree
printf 'Actinidia complete-control reveal inputs frozen (%s): %s\n' "$label_status" "$result_root"
