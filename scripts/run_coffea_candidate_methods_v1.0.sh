...
baseline_root=$project_root/results/baselines/coffea/v1.0
target=$normalized/target_et39/primary_chromosomes.genome.fa

[[ ${PLOIDYPATCH_BLIND_RUNNER:-} == 1 ]] || { echo "Coffea methods require blind runner" >&2; exit 65; }
[[ ${PLOIDYPATCH_NETWORK_ACCESS:-} == none ]] || { echo "Coffea methods require network=none" >&2; exit 66; }
...
for forbidden in /nas_data /holdout/evaluator_only /holdout/target_complete /holdout/truth /holdout/labels; do
    [[ ! -e $forbidden ]] || { echo "forbidden Coffea blind path visible: $forbidden" >&2; exit 68; }
done
...
for bundle in candidate_bua candidate_mauritiana; do
    for item in primary_chromosomes.genome.fa primary_chromosomes.gff3 \
        primary_chromosomes.lifton.gff3 provider.protein.fa; do
        [[ -s $normalized/$bundle/$item ]] || { echo "missing Coffea input: $bundle/$item" >&2; exit 69; }
    done
done
...
/usr/bin/time -v -o "$index_working/resource.time.txt" \
    "$miniprot" -t32 -d "$index_working/et39.mpi" "$target" \
    > "$index_working/stdout.log" 2> "$index_working/stderr.log"
(cd "$index_working" && find . -type f ! -name SHA256SUMS -printf '%P\0' | sort -z | \
...
run_miniprot() {
    ...
    /usr/bin/time -v -o "$working/logs/projection.time.txt" \
        ...
        "$index_root/et39.mpi" "$working/reference/protein.fa" \
    ...
    (cd "$working" && find . -type f ! -name SHA256SUMS -printf '%P\0' | sort -z | \
        xargs -0 sha256sum > SHA256SUMS && sha256sum -c SHA256SUMS >/dev/null)
    mv "$working" "$output"
}
...
for index in "${!pids[@]}"; do
    if ! wait "${pids[$index]}"; then
        printf 'Coffea candidate method failed: %s\n' "${labels[$index]}" >&2
        failed=1
    fi
done
[[ $failed -eq 0 ]] || exit 72
