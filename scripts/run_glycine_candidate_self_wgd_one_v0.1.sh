...
if [[ $# -lt 2 || $# -gt 3 ]]; then
    echo "usage: $0 PROJECT_ROOT blind|complete_control [support2|union]" >&2
    exit 2
fi

project_root=$(realpath "$1")
mode=$2
candidate_arm=${3:-support2}
...
case $mode in
    blind)
        base_gff=$project_root/benchmark/structure/copy_collapse_v0.1/gma_v21/annotation_copy_collapse_seed20260815/blind/perturbed.gff3
        ;;
    complete_control)
        base_gff=$bundle_root/primary_chromosomes.gff3
        ;;
    *)
...
case $candidate_arm in
    support2)
        consensus_root=$project_root/results/copy_collapse/consensus_glycine_v0.1/support2
        result_family=wgd_reanchor_glycine_v0.1
        candidate_source=method_consensus_support2
        split=post_holdout_exploratory_development
        formal_holdout_claim_allowed=false
        ;;
    union)
        consensus_root=$project_root/results/copy_collapse/consensus_union_glycine_dev_v0.1
        result_family=wgd_reanchor_union_glycine_dev_v0.1
        candidate_source=method_consensus_support1_union
        split=post_holdout_exploratory_development
        formal_holdout_claim_allowed=false
        ;;
    *)
...
candidate_gff=$consensus_root/$mode/candidate.gff3
result_root=$project_root/results/copy_collapse/$result_family/self_wgdi/$mode
working_root=${result_root}.working
prefix=gma_${candidate_arm}_candidate_$mode

for required in "$python_bin" "$diamond_bin" "$wgdi_bin" \
                "$syngap_env/bin/gffread" "$genome" "$fai" \
                "$base_gff" "$candidate_gff"; do
    if [[ ! -s $required ]]; then
        echo "missing or empty candidate self-WGD input: $required" >&2
        exit 1
    fi
done
if [[ -e $result_root || -e $working_root ]]; then
    echo "refusing to overwrite candidate self-WGD result: $result_root" >&2
    exit 1
fi
    ...
    "$working_root/pairs" "$working_root/selected" "$working_root/logs"
{
    ...
    printf 'split\t%s\n' "$split"
    printf 'formal_holdout_claim_allowed\t%s\n' "$formal_holdout_claim_allowed"
    ...
    printf 'diamond_threads\t48\n'
    printf 'wgdi_processes\t48\n'
    printf 'wgd_selector_module_sha256\t%s\n' \
        "$(sha256sum "$code_root/src/ploidypatch/wgd_candidate_select.py" | awk '{print $1}')"
...
/usr/bin/time -v -o "$working_root/logs/diamond_makedb.time.txt" \
    "$diamond_bin" makedb \
        --in "$working_root/input/$prefix.wgdi.pep.fa" \
...
/usr/bin/time -v -o "$working_root/logs/diamond_self.time.txt" \
    "$diamond_bin" blastp \
        --query "$working_root/input/$prefix.wgdi.pep.fa" \
        --db "$working_root/db/$prefix" \
...
for output in "$working_root/input/$prefix.wgdi.gff" \
              ...
              "$working_root/pairs/${prefix}.self_wgd_pairs.tsv.manifest.json" \
              "$working_root/selected/candidate.gff3" \
              "$working_root/selected/selection.tsv" \
              "$working_root/selected/stdout.json"; do
    if [[ ! -s $output ]]; then
        echo "missing or empty candidate self-WGD output: $output" >&2
        exit 1
    fi
done
(
...
printf 'Glycine candidate self-WGD arm frozen: %s\n' "$result_root"
