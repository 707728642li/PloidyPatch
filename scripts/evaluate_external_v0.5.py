...
from ploidypatch import external_evaluation as COMMON
from ploidypatch.artifact_manifest import verify_sha256sums as verify_artifact_tree

SCHEMA_VERSION = "ploidypatch.external_ranker_evaluation.v0.5"
...
H1_BOOTSTRAP_SEED = 20261011
H2_BOOTSTRAP_SEED = 20261012
GUARD_V03_BOOTSTRAP_SEED = 20261013
...
def verify_sha256sums(root: Path) -> dict[str, str]:
    return verify_artifact_tree(root, ignore_checksum_file=True)
...
def parse_input_roots(values: Sequence[str]) -> dict[str, Path]:
    roots: dict[str, Path] = {}
    for value in values:
        role, separator, raw_path = value.partition("=")
        if not separator or not role or not raw_path or role in roots:
            raise ValueError("--input-root requires unique ROLE=PATH values")
        root = Path(raw_path)
        if not root.is_dir() or root.is_symlink():
            raise ValueError(f"Input provenance root is missing or symlinked: {root}")
        roots[role] = root.resolve()
    return roots


def input_record(path: Path, roots: dict[str, Path]) -> dict[str, Any]:
    record: dict[str, Any] = {
    ...
    for role, root in roots.items():
        try:
            ...
            continue
        matches.append((len(root.parts), role, relative))
    if not matches:
        raise ValueError(f"Input is outside every declared provenance root: {path}")
    _, role, relative = max(matches, key=lambda item: (item[0], item[1]))
    record.update(
        {
...
def validate_custody(
    ...
    score_output = blind_outputs.get("scores", {})
    score_manifest_output = blind_outputs.get("score_manifest", {})
    if (
        custody.get("schema_version") != CUSTODY_SCHEMA
        or any(custody.get(field) is not False for field in forbidden_access)
        or not isinstance(score_output, dict)
        or score_output.get("sha256") != sha256(scores)
        or not isinstance(score_manifest_output, dict)
        or score_manifest_output.get("sha256") != sha256(score_manifest)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reveal a preregistered untouched PloidyPatch v0.5-framework external replication"
    )
    ...
    if (
        protocol_sums.get("protocol_manifest.json") != sha256(protocol_manifest_path)
        or protocol_manifest.get("schema_version")
        != "ploidypatch.external_holdout_protocol_freeze.v0.5"
        or protocol_manifest.get("policy_id") != POLICY_ID
        or protocol_manifest.get("model_version") != "PloidyPatch_ranker_v0.4"
        or protocol_manifest.get("composite_model_SHA256SUMS_sha256")
        != sha256(composite_root / "SHA256SUMS")
    ):
        raise ValueError("Protocol freeze does not bind the Actinidia policy and model")
    try:
        policy_relative = paths["policy"].resolve().relative_to(
            protocol_root.resolve()
        ).as_posix()
    except ValueError:
        raise ValueError("Policy must be the copy inside the protocol freeze") from None
    if protocol_sums.get(policy_relative) != sha256(paths["policy"]):
    ...
    if any(policy.get(key) != value for key, value in expected_policy.items()):
        raise ValueError("External v0.5 policy identity or seeds differ")
    ...
    if (
        ...
        raise ValueError("Composite v0.4 model freeze differs from protocol binding")
    ...
    report: dict[str, Any] = {
        ...
        "inputs": {
            role: input_record(path, input_roots)
            for role, path in paths.items()
        },
        "counts": {
    ...
    if output.exists() or partial.exists():
        raise FileExistsError("Refusing to overwrite external v0.5 evaluation")
