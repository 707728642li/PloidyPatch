...
pair_root=$project_root/results/evaluator/actinidia/v0.5/truth_pairs
...
seed=20261010
maximum_count=800
result_root=$project_root/benchmark/structure/copy_collapse_v0.5/red5_ps1_1.69.0/annotation_copy_collapse_seed${seed}
working_root=${result_root}.working
self_relative=scripts/run_actinidia_copy_collapse_benchmark_v0.5.sh
...
for required in "$python_bin" "$source_root/EVALUATOR_SHA256SUMS" "$source_gff" "$source_genome" \
    ...
    [[ -s $required ]] || { echo "missing Actinidia benchmark prerequisite: $required" >&2; exit 1; }
done
...
[[ $(policy_value policy_id) == ploidypatch_actinidia_external_validation_v0.5 \
    ...
    && $(policy_value minimum_target_chromosomes) == 22 \
    && $(policy_value minimum_events_per_complexity_bin) == 20 ]] || {
    echo "Actinidia benchmark policy identity, seed, count, or evaluability gates differ" >&2; exit 1;
}
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Actinidia v0.5 benchmark" >&2; exit 1;
}
...
[[ $available_pairs -ge 0 ]] || { echo "Actinidia truth-pair table lacks a header" >&2; exit 1; }
...
report = {
    "schema_version": "ploidypatch.actinidia_external_evaluability.v0.5",
    "events": 0,
    ...
    "reason": "zero_pairs_pass_frozen_self_and_two_evaluator_group_intersection",
...
PY
    ...
    finalize_tree
    printf 'Actinidia v0.5 benchmark is not evaluable under frozen pair rules: %s\n' "$result_root"
    exit 0
fi
...
for required in "$selected" "$selected.manifest.json" "$selection_decisions"; do
    [[ -s $required ]] || { echo "missing frozen Actinidia pair-sampling artifact: $required" >&2; exit 1; }
done
selected_count=$(( $(wc -l < "$selected") - 1 ))
[[ $selected_count -gt 0 && $selected_count -le $maximum_count ]] || {
    echo "Actinidia selected pair count is outside 1..$maximum_count" >&2; exit 1;
}
...
[[ -s $perturbed && -s $truth && -s $working_root/blind/manifest.json ]] || {
    echo "Actinidia perturbation lacks blind or evaluator output" >&2; exit 1;
}
...
if [[ $formal_outcome == invalid_run ]]; then
    echo "Actinidia benchmark sentinel violation was frozen as invalid_run: $result_root" >&2
    exit 1
fi
printf 'Actinidia v0.5 copy-collapse benchmark frozen (%s): %s\n' "$formal_outcome" "$result_root"
