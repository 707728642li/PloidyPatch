...
miniprot_root=$project_root/results/baselines/populus_v0.4/miniprot
...
result_root=$project_root/results/copy_collapse/external/populus_v0.4_blind_self_wgd
working_root=${result_root}.working
prefix=ptr_v4_union_candidate_blind

[[ ${PLOIDYPATCH_BLIND_RUNNER:-} == 1 ]] || {
    echo "Populus self-WGD must run inside the frozen blind runner" >&2; exit 1;
}
...
for required in "$python_bin" "$diamond_bin" "$wgdi_bin" \
    ...
    "$execution_root/SHA256SUMS" "$code_root/scripts/build_wgdi_source_alias_gff.py"; do
    [[ -s $required ]] || { echo "missing Populus blind self-WGD input: $required" >&2; exit 1; }
done
verify_tree "$protocol_root"
verify_tree "$execution_root"
verify_tree "$miniprot_root"
verify_tree "$method_root"
verify_implementation scripts/run_populus_blind_union_self_wgd_v0.4.sh
...
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Populus blind self-WGD" >&2; exit 1;
}
...
cp /proc/self/mountinfo "$working_root/freeze/blind_runner.mountinfo"
{
    ...
    printf 'mode\tblind\nsplit\tuntouched_confirmatory_external_v0.4\n'
    ...
    printf 'wgd_event\tsalicoid_wgd_blind_recomputed\nmin_block_pairs\t20\n'
...
/usr/bin/time -v -o "$working_root/logs/pair_inference.time.txt" \
    ...
    --wgd-event salicoid_wgd_blind_recomputed --min-block-pairs 20 \
...
for output in "$working_root/input/$prefix.wgdi.gff" \
    ...
    [[ -s $output ]] || { echo "missing Populus blind self-WGD output: $output" >&2; exit 1; }
done
(
    cd "$working_root"
...
printf 'Populus blind candidate self-WGD frozen: %s\n' "$result_root"
