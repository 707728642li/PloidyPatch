...
method_root=$project_root/results/copy_collapse/external/populus_v0.4_method_trio
...
self_wgd_root=$project_root/results/copy_collapse/external/populus_v0.4_blind_self_wgd
...
result_root=$project_root/results/copy_collapse/external/populus_v0.4_blind_rankings
working_root=${result_root}.working

[[ ${PLOIDYPATCH_BLIND_RUNNER:-} == 1 ]] || {
    echo "Populus scoring must run inside the frozen blind runner" >&2; exit 1;
}
...
for required in "$python_bin" "$base_gff" "$method_root/SHA256SUMS" \
    ...
    [[ -s $required ]] || { echo "missing Populus blind ranking input: $required" >&2; exit 1; }
done
for root in "$method_root" "$self_wgd_root" "$protocol_root" "$execution_root" \
            "$composite_root"; do
    verify_tree "$root"
done
verify_implementation scripts/score_populus_candidates_blind_v0.4.sh
...
if len(policy) != len(rows):
    raise SystemExit("duplicate Populus policy field")
expected_composite = sha(root / "SHA256SUMS")
if (
    policy.get("policy_id") != "ploidypatch_populus_external_validation_v0.4"
    or policy.get("composite_model_sha256sums_sha256") != expected_composite
    ...
    raise SystemExit("Populus policy/composite binding differs")
...
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Populus blind rankings" >&2; exit 1;
}
...
if not rows or "v04_automatic_approval" not in fields:
    raise SystemExit("v0.4 score table is empty or lacks approval boundary")
if any("truth" in field.lower() or "label" in field.lower() for field in fields):
    raise SystemExit("truth/label field leaked into blind v0.4 scores")
if any(row["v04_automatic_approval"] != "0" for row in rows):
    raise SystemExit("v0.4 emitted an automatic approval")
...
if (
    ...
    raise SystemExit("v0.4 production guard failed its blind safety contract")
PY
...
for output in scores/v03.tsv scores/v03.tsv.manifest.json scores/v04.tsv \
    ...
    [[ -s $working_root/$output ]] || { echo "missing Populus blind score output: $output" >&2; exit 1; }
done
{
...
printf 'Populus truth-blind v0.4 rankings frozen: %s\n' "$result_root"
