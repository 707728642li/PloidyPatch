...
result_root=$project_root/results/copy_collapse/holdout/maize_v2_union_self_wgd/blind
working_root=${result_root}.working
prefix=zma_union_candidate_blind

[[ -s $method_root/SHA256SUMS ]] || { echo "unfrozen maize blind candidates" >&2; exit 1; }
...
for required in "$python_bin" "$diamond_bin" "$wgdi_bin" \
    ...
    [[ -s $required ]] || { echo "missing maize self-WGD input: $required" >&2; exit 1; }
done
if [[ -e $result_root || -e $working_root ]]; then
    echo "refusing to overwrite maize self-WGD" >&2; exit 1
fi
...
for output in "$working_root/input/$prefix.wgdi.gff" \
    ...
    [[ -s $output ]] || { echo "missing maize self-WGD output: $output" >&2; exit 1; }
done
(
    cd "$working_root"
...
printf 'maize blind candidate self-WGD frozen: %s\n' "$result_root"
