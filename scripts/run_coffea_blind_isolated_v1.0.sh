...
namespace_entry() {
    ...
    for required in /holdout/shared_target /holdout/candidate_only /holdout/blind_benchmark \
        /holdout/role_manifest.json /holdout/role_manifest.tsv /holdout/SHA256SUMS; do
...
[[ -f $pipeline && $holdout_id == coffea_et39_v1.0 ]] || exit 67
...
args+=(--ro-bind "$execution" /frozen/execution --ro-bind "$execution/source" /frozen/source
 ...
 --ro-bind "$role_root/SHA256SUMS" /holdout/SHA256SUMS --chdir "$internal"
 /usr/bin/bash /frozen/source/scripts/run_coffea_blind_isolated_v1.0.sh
...
payload={'schema_version':'ploidypatch.coffea_core_h1_mount_manifest.v1.0','holdout_id':sys.argv[3],
...
PYTHONPATH="$execution/source/src" "$dev_python" "$execution/source/scripts/finalize_coffea_blind_custody_v1.0.py" \
