...
projection_root=$project_root/results/baselines/miniprot_brassica_v0.1
miniprot_gff=$projection_root/raw/miniprot.gff3
protein_map=$projection_root/reference/brassica_progenitors.map.tsv
result_root=$project_root/results/copy_collapse/miniprot_brassica_v0.1
...
for required in "$python_bin" "$source_gff" "$perturbed_gff" "$truth" \
    ...
    if [[ ! -s $required ]]; then
        echo "missing or empty copy-collapse miniprot input: $required" >&2
        exit 1
    fi
done
if [[ -e $result_root || -e $working_root ]]; then
    echo "refusing to overwrite copy-collapse miniprot result: $result_root" >&2
    exit 1
fi
...
if ! grep -q '"grade": "pass"' "$working_root/score.json"; then
    echo "copy-collapse miniprot score quality gate failed" >&2
    exit 1
fi
...
for output in "$working_root/blind/candidate.gff3" \
    ...
    if [[ ! -s $output ]]; then
        echo "missing or empty copy-collapse miniprot output: $output" >&2
        exit 1
    fi
done
(
...
printf 'copy-collapse miniprot baseline frozen: %s\n' "$result_root"
