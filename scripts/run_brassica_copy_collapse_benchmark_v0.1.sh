...
pair_root=$project_root/results/evidence/homeolog_pairs/brassica_v0.1
pair_tsv=$pair_root/bna_daae.AC.homeolog_pairs.tsv
pair_manifest=${pair_tsv}.manifest.json
count=800
seed=20260814
result_root=$project_root/benchmark/structure/copy_collapse_v0.1/bna_daae/annotation_copy_collapse_seed${seed}
working_root=${result_root}.working

for required in "$python_bin" "$source_gff" "$pair_tsv" "$pair_manifest"; do
    if [[ ! -s $required ]]; then
        echo "missing or empty copy-collapse prerequisite: $required" >&2
        exit 1
    fi
done
if [[ -e $result_root || -e $working_root ]]; then
    echo "refusing to overwrite copy-collapse benchmark: $result_root" >&2
    exit 1
fi
...
if ! cmp -s "$source_gff" "$restored"; then
    echo "copy-collapse restoration is not byte-identical to source" >&2
    exit 1
fi
...
for score in "$working_root/evaluator/validation/score_noop.json" \
    ...
    if ! grep -q '"grade": "pass"' "$score"; then
        echo "copy-collapse evaluator quality gate failed: $score" >&2
        exit 1
    fi
done
...
for output in "$perturbed" "$working_root/blind/manifest.json" "$truth" \
              ...
              "$working_root/evaluator/validation/score_oracle.json"; do
    if [[ ! -s $output ]]; then
        echo "missing or empty copy-collapse output: $output" >&2
        exit 1
    fi
done
(
...
printf 'copy-collapse benchmark frozen: %s\n' "$result_root"
