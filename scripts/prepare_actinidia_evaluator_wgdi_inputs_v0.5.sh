...
source_root=$project_root/data/derived/external_inputs/actinidia/v0.5
...
result_root=$project_root/data/derived/external_evaluator/actinidia_v0.5_wgdi_inputs
working_root=${result_root}.working
self_relative=scripts/prepare_actinidia_evaluator_wgdi_inputs_v0.5.sh
...
for required in "$python_bin" "$gffread_bin" "$parallel_bin" \
    ...
    [[ -s $required ]] || { echo "missing Actinidia evaluator WGDI-input prerequisite: $required" >&2; exit 1; }
done
...
declare -A prefix=(
    [target_red5]=red5 [evaluator_rhododendron]=rhs [evaluator_diospyros]=dol
)
species=(target_red5 evaluator_rhododendron evaluator_diospyros)
for name in "${species[@]}"; do
    ...
    for required in "$bundle/primary_chromosomes.gff3" \
        ...
        [[ -s $required ]] || { echo "missing normalized Actinidia evaluator source: $required" >&2; exit 1; }
    done
done
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Actinidia evaluator WGDI inputs" >&2; exit 1;
}
...
trap record_invalid ERR
{
    ...
    printf 'prefix_target\tred5\nprefix_evaluator_rhododendron\trhs\nprefix_evaluator_diospyros\tdol\n'
    printf 'target_peptide_policy\texact_chromosome_GFF_CDS_protein_id_whitelist\n'
...
prepare_one() {
    ...
    mkdir -p "$out"
    if [[ $name == target_red5 ]]; then
        local whitelisted=$bundle/chromosome_whitelist.pep.fa
        local whitelist_manifest=$bundle/chromosome_whitelist.manifest.json
        [[ -s $whitelisted && -s $whitelist_manifest ]] || {
            echo "missing frozen Red5 chromosome protein_id whitelist" >&2; return 1;
        }
        cp -- "$whitelisted" "$out/$short.all.pep.fa"
        printf 'Red5 peptide input copied from exact chromosome protein_id whitelist\n' \
            > "$out/gffread.stdout.log"
        : > "$out/gffread.stderr.log"
    else
...
for short in red5 rhs dol; do
    for suffix in wgdi.gff wgdi.lens wgdi.pep.fa wgdi_inputs.manifest.json representatives.tsv; do
        [[ -s $working_root/$short/$short.$suffix ]] || {
            echo "missing Actinidia evaluator WGDI artifact: $short.$suffix" >&2; exit 1;
        }
    done
    genes=$(wc -l < "$working_root/$short/$short.wgdi.gff")
    chromosomes=$(wc -l < "$working_root/$short/$short.wgdi.lens")
    case $short in
        red5) expected_genes=32950; expected_chromosomes=29 ;;
        rhs) expected_genes=30419; expected_chromosomes=13 ;;
        dol) expected_genes=29268; expected_chromosomes=15 ;;
        *) echo "unexpected WGDI prefix: $short" >&2; exit 1 ;;
    esac
    [[ $genes -eq $expected_genes && $chromosomes -eq $expected_chromosomes ]] || {
        echo "$short WGDI input counts differ from syntax-only frozen diagnostics" >&2; exit 1;
    }
...
printf 'Actinidia evaluator WGDI inputs frozen: %s\n' "$result_root"
