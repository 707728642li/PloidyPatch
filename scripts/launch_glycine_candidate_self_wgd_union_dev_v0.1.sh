...
result_root=$project_root/results/copy_collapse/wgd_reanchor_union_glycine_dev_v0.1/self_wgdi
state_root=$project_root/logs/copy_collapse/wgd_reanchor_union_glycine_dev_v0.1
...
if [[ -e $state_root || -e $result_root/blind || \
    ...
    echo "refusing to reuse union self-WGD launcher state or results" >&2
    exit 2
fi
mkdir -p "$state_root"
...
for mode in blind complete_control; do
    nohup setsid env PLOIDYPATCH_CODE_COMMIT="${PLOIDYPATCH_CODE_COMMIT:-unavailable_server_mirror}" \
        bash "$runner" "$project_root" "$mode" union \
...
for mode in blind complete_control; do
    ...
    if ! kill -0 "$pid"; then
        echo "union self-WGD $mode launcher exited during startup" >&2
        exit 1
    fi
    printf '%s_pid\t%s\n' "$mode" "$pid"
done
