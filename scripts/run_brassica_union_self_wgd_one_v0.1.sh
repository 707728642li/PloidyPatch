...
result_root=$project_root/results/copy_collapse/model_development/brassica_union_self_wgd_v0.1/$mode
working_root=${result_root}.working
prefix=bna_union_candidate_$mode

if [[ ! -s $method_root/SHA256SUMS ]]; then
    echo "Brassica method-trio result is not frozen" >&2
    exit 1
fi
...
for required in "$python_bin" "$diamond_bin" "$wgdi_bin" \
    ...
    if [[ ! -s $required ]]; then
        echo "missing or empty Brassica candidate self-WGD input: $required" >&2
        exit 1
    fi
done
if [[ -e $result_root || -e $working_root ]]; then
    echo "refusing to overwrite Brassica candidate self-WGD result" >&2
    exit 1
fi
    ...
    "$working_root/pairs" "$working_root/selected" "$working_root/logs"
{
    ...
    printf 'duplication_context\tBrassica_napus_recent_allopolyploid_plus_mesopolyploid_blocks\n'
...
/usr/bin/time -v -o "$working_root/logs/pair_inference.time.txt" \
    "$python_bin" -m ploidypatch.cli evidence infer-self-wgd-pairs \
        ...
        --wgd-event brassica_napus_duplication_context \
        --min-block-pairs 20 \
...
for output in "$working_root/input/$prefix.wgdi.gff" \
    ...
    if [[ ! -s $output ]]; then
        echo "missing Brassica self-WGD output: $output" >&2
        exit 1
    fi
done
(
...
printf 'Brassica candidate self-WGD frozen: %s\n' "$result_root"
