...
result_root=$project_root/results/evaluator/actinidia/v0.5/wgdi
working_root=${result_root}.working
self_relative=scripts/run_actinidia_evaluator_wgdi_v0.5.sh
...
for required in "$input_root/SHA256SUMS" "$protocol_root/SHA256SUMS" \
    ...
    [[ -s $required ]] || { echo "missing Actinidia WGDI prerequisite: $required" >&2; exit 1; }
done
...
[[ ! -e $result_root && ! -e $working_root ]] || {
    echo "refusing to overwrite Actinidia evaluator WGDI" >&2; exit 1;
}
...
trap record_invalid ERR
{
    ...
    printf 'query\tActinidia_chinensis_Red5_PS1_1.69.0_EnsemblPlants45\n'
    printf 'target_event\tActinidia_specific_Ad_alpha\n'
    printf 'self_comparison\ttrue\nreferences\tRhododendron_simsii_GCA_014282245.1,Diospyros_oleifera_v1.0\n'
...
printf '%s\n' red5 rhs dol | "$parallel_bin" --jobs 3 --delay 1 make_db {}

run_diamond() {
    local pair=$1 ref=$2
    /usr/bin/time -v -o "$working_root/logs/$pair.diamond.time.txt" \
        "$diamond_bin" blastp --query "$input_root/red5/red5.wgdi.pep.fa" \
...
printf '%s\t%s\n' red5_self red5 red5_vs_rhs rhs red5_vs_dol dol \
    | "$parallel_bin" --jobs 2 --delay 1 --colsep '\t' run_diamond {1} {2}

for entry in red5_self:red5 red5_vs_rhs:rhs red5_vs_dol:dol; do
    pair=${entry%%:*}; ref=${entry#*:}; config=$working_root/config/$pair.conf
    {
        printf '[collinearity]\n'
        printf 'gff1 = %s\ngff2 = %s\n' "$input_root/red5/red5.wgdi.gff" \
            "$input_root/$ref/$ref.wgdi.gff"
        printf 'lens1 = %s\nlens2 = %s\n' "$input_root/red5/red5.wgdi.lens" \
...
printf 'Actinidia evaluator WGDI frozen: %s\n' "$result_root"
