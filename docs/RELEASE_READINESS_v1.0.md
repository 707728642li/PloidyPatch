# PloidyPatch v1.0 release readiness
...
PloidyPatch v1.0 is the stable software and paper release. Its registry package
version is `1.0.0`, and it includes the
review workbench, packaging, documentation and engineering improvements without
retraining a ranker, changing a scientific threshold, reinterpreting a frozen
external result, or making candidate approval automatic.
...
- the Bioconda recipe SHA must equal the final canonical v1.0 source archive;
- the PyPI and Bioconda packages must be built from the same immutable v1.0
  source archive.
...
The exact v1.0 archive DOI is `10.5281/zenodo.21875561`; the project-level
...
The final 2026-08-18 clean public tree produced the following results:

- Windows release suite: **639 passed, 10 platform-specific skips, 0 failed**;
- Linux release suite: **648 passed, 1 platform-specific skip, 0 failed**;
- GitHub Actions passed the Python 3.11--3.13 matrix, package, container,
  macOS, report-quality and CodeQL checks on the public root commit;
- report JavaScript syntax, light/dark presentation, keyboard focus,
  reduced-motion behavior and packaged-asset checks passed in CI;
- every third-party workflow action is pinned to a full 40-character commit
  SHA;
...
- `twine check` passed for both Python distribution formats;
- the noarch Conda package built and passed import, CLI-version, help and
  byte-identical reversion tests;
- the canonical sdist excludes the external Bioconda recipe while retaining
  the packaged HTML/CSS/JavaScript report assets, avoiding checksum
  self-reference.
